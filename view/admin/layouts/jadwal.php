        ...
        if (isset($_GET['status'])) {
            if ($_GET['status'] == 1) {
                ...
                <div class="flex bg-green-300 px-4 py-1 w-full rounded-lg items-center gap-4 text-slate-800 font-medium">
                    ...
                    <p>Data Jadwal <strong>berhasil</strong> ditambah!</p>
                </div>
                '; 
            } if ($_GET['status'] == 2) {
                echo '
                <div class="flex bg-green-300 px-4 py-1 w-full rounded-lg items-center gap-4 text-slate-800 font-medium">
                    ...
                    <p>Data Jadwal <strong>berhasil</strong> diubah!</p>
                </div>
                ';
            } if ($_GET['status'] == 3) {
                echo '
                <div class="flex bg-red-300 px-4 py-1 w-full rounded-lg items-center gap-4 text-slate-800 font-medium">
                    ...
                    <p>Data Jadwal <strong>berhasil</strong> dihapus!</p>
                </div>
                ';
            } if ($_GET['status'] == 4) {
                echo '
...
<form action="" method="post" class="flex justify-between">
        ...
        <a href="?page=tambah_jadwal" class="basis-1/4 bg-slate-800 py-2 px-4 rounded-lg text-white font-medium flex gap-2 items-center">
            <i>
    ...
    <table class="pt-4 w-full rounded-t-md overflow-hidden text-sm">
        <thead class="bg-slate-800 text-white">
            <tr>
                ...
                <th class="py-2 px-4 text-start">ID Jadwal</th>
        ...
        <tbody>
            <?php
                if (isset($cari)) {
                    $jumlah = "SELECT * FROM jadwal WHERE id_jadwal LIKE '%$cari%' OR hari LIKE '%$cari%' OR start LIKE '%$cari%' OR end LIKE '%$cari%'";
                } else {
                    $jumlah = "SELECT * FROM jadwal";
                }
                ...
                if (isset($cari)) {
                    $sql = "SELECT * FROM jadwal WHERE id_jadwal LIKE '%$cari%' OR hari LIKE '%$cari%' OR start LIKE '%$cari%' OR end LIKE '%$cari%' ORDER BY id_jadwal ASC LIMIT $start, $limit";
                }
                ...
                while ($data = mysqli_fetch_array($query)) {
                    ...
                    echo "<td class='px-4 py-2'>".$data['id_jadwal']."</td>";
                    ...
                    echo "<td class='px-4 py-2 flex items-center gap-2'>
                        <a href='?page=ubah_jadwal&id=$data[id_jadwal]' class='px-2 py-1.5 text-blue-500'>
                            <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor' class='w-4 h-4'>
                                <path d='M21.731 2.269a2.625 2.625 0 0 0-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 0 0 0-3.712ZM19.513 8.199l-3.712-3.712-8.4 8.4a5.25 5.25 0 0 0-1.32 2.214l-.8 2.685a.75.75 0 0 0 .933.933l2.685-.8a5.25 5.25 0 0 0 2.214-1.32l8.4-8.4Z' />
                                <path d='M5.25 5.25a3 3 0 0 0-3 3v10.5a3 3 0 0 0 3 3h10.5a3 3 0 0 0 3-3V13.5a.75.75 0 0 0-1.5 0v5.25a1.5 1.5 0 0 1-1.5 1.5H5.25a1.5 1.5 0 0 1-1.5-1.5V8.25a1.5 1.5 0 0 1 1.5-1.5h5.25a.75.75 0 0 0 0-1.5H5.25Z' />
                            </svg>
                        </a>
                        <a href='?page=jadwal&delete=5&id=$data[id_jadwal]' class='text-red-400'>
                            <svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='currentColor' class='w-4 h-4'>
