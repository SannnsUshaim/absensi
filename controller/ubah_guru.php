        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (!empty($_FILES['image']['name']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($image_temp, $target_file)) {
                    ...
                    $query_update_murid = mysqli_query($koneksi, "UPDATE guru SET nama='$nama', alamat='$alamat', no_telp='$no_telp', kelas='$kelas', walas='$walas',mapel='$mapel' WHERE id='$id'");
                    ...
                    if ($query_update_murid && $query_update_user) {
                        header('Location: ../?page=data_guru&status=2'); // status=2 untuk update berhasil
                        exit;
                    } else {
